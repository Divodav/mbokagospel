...
import { Play, Pause, Disc, Share2, ArrowLeft, Music, Clock, ExternalLink, User } from "lucide-react";
...
const ShareAlbum = () => {
    ...
    useEffect(() => {
        const fetchAlbum = async () => {
            ...
            try {
                ...
                const albumTitle = albumData.title || albumData.name;

                // Update document metadata for SEO
                document.title = `${albumTitle} - ${albumData.artist_name || 'Album'} | MbokaGospel`;

                // Update meta tags
                updateMetaTag('og:title', `${albumTitle} - ${albumData.artist_name || 'Album'}`);
                updateMetaTag('og:description', `Découvrez l'album "${albumTitle}" sur MbokaGospel`);
                updateMetaTag('og:image', albumData.cover_url || albumData.cover || '');
                updateMetaTag('og:url', window.location.href);
                updateMetaTag('og:type', 'music.album');
    ...
    if (!album) {
        return (
            <div className="min-h-screen bg-[#080405] text-white flex flex-col items-center justify-center p-8">
                <Disc size={64} className="text-gray-600 mb-4" />
                <h1 className="text-2xl font-bold mb-2">Album introuvable</h1>
                <p className="text-gray-500 mb-8">Cet album n'existe pas ou n'est plus disponible.</p>
    ...
    const albumCover = album.cover_url || album.cover;
                        ...
                        <div className="w-48 h-48 rounded-2xl overflow-hidden shadow-2xl shrink-0">
                            {albumCover ? (
                                ...
                                <div className="w-full h-full bg-white/10 flex items-center justify-center">
                                    <Disc size={60} className="text-gray-600" />
                                </div>
                            )}
                        </div>

                        <div className="text-center md:text-left flex-1">
                            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-primary mb-2">ALBUM</p>
                            <h1 className="text-3xl md:text-4xl font-black tracking-tight mb-2">{albumTitle}</h1>
                            <div className="flex items-center justify-center md:justify-start gap-2 text-gray-400 mb-4">
                                <User size={14} />
                                <span className="font-medium">{album.artist_name || 'Artiste'}</span>
                                <span>•</span>
